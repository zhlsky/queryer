...
struct Projection<'a>(&'a SelectItem);
struct Offset<'a>(&'a SqlOffset);
struct BinaryOperator(SqlBinaryOperator);
struct Expression(Box<SqlExpr>);
struct Order<'a>(&'a OrderByExpr);
struct Source<'a>(&'a [TableWithJoins]);
struct Limit<'a>(&'a SqlExpr);
...
impl TryFrom<BinaryOperator> for Operator {
    ...
    fn try_from(operator: BinaryOperator) -> Result<Self, Self::Error> {
        match operator.0 {
            ...
            SqlBinaryOperator::Xor => Ok(Self::Xor),
...
impl<'a> TryFrom<Order<'a>> for (String, bool) {
    ...
    fn try_from(order: Order) -> Result<Self, Self::Error> {
        let name = match &order.0.expr {
            SqlExpr::Identifier(ident) => ident.to_string(),
            expr => {
                return Err(anyhow!(
